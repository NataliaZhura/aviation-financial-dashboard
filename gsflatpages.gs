...
function flatBuildDashboardData_(year, periodType, quarter) {
  ...
  const latestYear = year || flatLatestYear_(rows);
...
function flatBuildDashboardDataV2_(year, periodType, quarter) {
  ...
  const history = isQuarter ? [] : [2022, 2023, 2024].map(function(itemYear) {
  ...
  const fcfQuarterly = flatQuarterMetricHistory_(rows, 'MODEL_FCF', selectedYear).map(function(row) {
    return { quarter: row.quarter, value: row.value, valueMln: row.value / 1000 };
  });
  ...
  base.kpis = {
    revenue: revenue, revenueFormatted: flatFormatMillions_(revenue),
    netProfit: netProfit, netProfitFormatted: flatFormatMillions_(netProfit),
    ...
    ebitda: ebitda, ebitdaFormatted: flatFormatMillions_(ebitda),
    assets: assets, assetsFormatted: flatFormatMillions_(assets),
    liabilities: liabilities, liabilitiesFormatted: flatFormatMillions_(liabilities),
    ...
    equity: equity, equityFormatted: flatFormatMillions_(equity),
    netAssets: metric('NET_ASSETS'), netAssetsFormatted: money('NET_ASSETS'),
    cash: cash, cashFormatted: flatFormatMillions_(cash),
    creditDebt: creditDebt, creditDebtFormatted: flatFormatMillions_(creditDebt),
    ...
    netMargin: revenue && netProfit !== null ? netProfit / revenue : null, netMarginFormatted: revenue && netProfit !== null ? flatFormatPercent_(netProfit / revenue) : '-',
    ...
    annualPayment: annualPayment, annualPaymentFormatted: flatFormatMillions_(annualPayment),
    leasePayment: leasePayment, leasePaymentFormatted: flatFormatMillions_(leasePayment),
    modelFcf: modelFcf, modelFcfFormatted: flatFormatMillions_(modelFcf),
    officialCfads: '-', officialDcr: '-',
    monthlyMandatoryPayment: monthlyMandatoryPayment, monthlyMandatoryPaymentFormatted: flatFormatMillions_(monthlyMandatoryPayment),
    ...
    dcrFcf: annualPayment && modelFcf !== null ? modelFcf / annualPayment : null,
    dcrFcfFormatted: annualPayment && modelFcf !== null ? flatFormatRatio_(modelFcf / annualPayment) : '-',
    leaseCoverage: leasePayment && modelFcf !== null ? modelFcf / leasePayment : null,
    leaseCoverageFormatted: leasePayment && modelFcf !== null ? flatFormatRatio_(modelFcf / leasePayment) : '-',
